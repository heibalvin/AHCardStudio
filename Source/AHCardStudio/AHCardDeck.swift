...
enum AHCardDeckVisibilityPolicy {
    case allFolded
    case allVisible
}

enum AHCardDeckInventoryPolicy {
    case lilo       // Last In Last Out
}
...
class AHCardDeck: CustomStringConvertible {
    var name: String
    var cards: [AHCard]
    var visibility: AHCardDeckVisibilityPolicy
    var policy: AHCardDeckInventoryPolicy
    
    
    var description: String {
        return "\(self.name) (\(self.cards.count)): \(cards.map { "\($0)" }.joined(separator: " "))"
    }
    
    init(name: String, visibility: AHCardDeckVisibilityPolicy = .allFolded, policy: AHCardDeckInventoryPolicy = .lilo) {
        self.name = name
        self.cards = []
    ...
    func generate(_ type: AHCardDeckType = .standard52) {
        clear()
